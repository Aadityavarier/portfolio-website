...
const Contact = () => {
  ...
  return (
    <section id="contact" className="section relative" aria-label="Contact Information" ref={ref}>
      {/* Overlay */}
      <div
        style={{
          position: 'absolute',
      ...
      <div className="relative z-1 w-full h-full flex flex-col justify-center items-center text-center">
        ...
        <div className="max-w-4xl w-full px-4 mb-20">
          ...
          >
            {links.map((link) => {
              let ariaLabel = "";
              if (link.label === 'GitHub') ariaLabel = "Visit Aaditya Varier's GitHub profile";
              else if (link.label === 'LinkedIn') ariaLabel = "Visit Aaditya Varier's LinkedIn profile";
              else if (link.label === 'Email') ariaLabel = "Send an email to Aaditya Varier";
              return (
                <a
                  key={link.label}
                  href={link.url}
      ...
      >
        ...
        <div className="hidden md:flex gap-4">
          {[
            { icon: githubIcon, url: links[0].url, label: "Visit Aaditya Varier's GitHub profile" },
            { icon: linkedinIcon, url: links[1].url, label: "Visit Aaditya Varier's LinkedIn profile" },
            { icon: mailIcon, url: links[2].url, label: "Send an email to Aaditya Varier" }
          ].map((item, i) => (
            <a
              key={i}
              href={item.url}
